...
#include "osx/UpdaterOsx.h"
#elif defined(TARGET_WINDOWS)
#include "win32/UpdaterWindows.h"
#endif
...
CUpdateHandler &CUpdateHandler::Get()
{
  return sUpdateHandler;
}

void CUpdateHandler::OnSettingAction(const CSetting *setting)
{
  if (setting == NULL)
    return;
...
void CUpdateHandler::OnSettingChanged(const CSetting *setting)
{
  if (setting == NULL)
    return;
  ...
  {
    SetAutoUpdateEnabled(((CSettingBool*)setting)->GetValue());
  }
}
...
void CUpdateHandler::OnSettingsLoaded()
{
  if (HasExternalSettingsStorage())
    CSettings::Get().SetBool("updates.enableautoupdate", GetAutoUpdateEnabled());
}
